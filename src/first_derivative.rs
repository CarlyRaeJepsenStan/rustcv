...
pub fn calc_first_derivative_image_horizontal(input: &DynamicImage) -> ImageBuffer<Luma<u8>, Vec<u8>> {
    let (width, height) = input.dimensions();
    let image = to_greyscale(input);

    let mut output_hor = image.clone();
    ...
    for i in 0..(height-1) {
        for j in 0..(width-1) {
            ...
            out_pix = output_hor.get_pixel_mut(j, i);
            out_pix.0[0] = diff;
        }
    }
